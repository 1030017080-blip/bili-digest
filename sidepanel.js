 ...
 * SIDE PANEL LOGIC
 *
 * Handles the UI for Bili Digest: video detection, transcript analysis,
 * rendering results, and export features.
 */

const DEBUG = false;

// True when sidepanel.html runs inside the in-page overlay iframe (content
// script) rather than as Chrome's native side panel. Frame-hosted panels must
// not close themselves or follow tab activation — the host content script
// controls lifecycle.
const YTD_IN_IFRAME = (() => {
  try {
...
/**
 * If a display block ends without dialogue punctuation, append a sensible
 * sentence end so Bilibili's hard-cut caption blocks read more naturally.
 * Non-CJK trailing text gets an ASCII period so English captions stay proper.
 */
...
/**
 * B 站 AI 字幕在纯 BGM / 无对白片段会插入「♪ 音乐 ♪」「[音乐]」之类的占位符。
 * 这类条目不含真实对话，应整体丢弃，避免刷屏。
 */
function isPlaceholderSubtitle(text) {
...
function groupTranscriptEntries(entries, limits = TRANSCRIPT_SEGMENT_LIMITS) {
  ...
  entries.forEach((entry, entryIndex) => {
    ...
    if (!text || isPlaceholderSubtitle(text)) return;
  ...
  const flush = () => {
    ...
    const text = ensureSentenceEnding(normalizeCaptionText(current.text));
    grouped.push({
  ...
  pieces.forEach((piece) => {
    if (!current) current = { start: piece.start, text: "", lastStart: piece.start };
    // ASR/AI 字幕常无原生标点（会连成一大串），用"时间停顿"代替标点判断句界：
    // 与上一条的间隔越长，越可能是句末（长停顿时切出完整句，句尾由 flush 补齐句号）。
    const gapToPrev = Math.max(0, piece.start - (current.lastStart ?? current.start));
    current.lastStart = piece.start;
    ...
    const pauseEndsSentence = gapToPrev >= 1.2 && comfortablySized;
    ...
    if (
      (atNaturalBoundary && (comfortablySized || elapsed >= 8 || pauseEndsSentence)) ||
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Subtitles are free via the native Bilibili API. Only AI features need a key.
  if (configStatus && configStatus.hasAiKey === false) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "startDigestFromButton") {
    // The overlay iframe can't resolve the active Bilibili tab via chrome.tabs,
    // so the host content script hands us the metadata directly. Use it to show
    // the header video info immediately; startDigest() still fetches/renders the
    // transcript keyed by videoId (cache is served when already seen).
    if (YTD_IN_IFRAME && message.videoId) {
      if (message.videoTitle) currentVideoTitle = message.videoTitle;
      if (message.channelName) currentChannelName = message.channelName;
      if (message.videoUrl) currentVideoUrl = message.videoUrl;
      const fallbackUrl = `https://www.bilibili.com/video/${message.videoId}`;
      const vid = extractVideoId(message.videoUrl || fallbackUrl);
      const videoInfo = document.getElementById("videoInfo");
      if (videoInfo && (currentVideoTitle || currentChannelName)) {
        document.getElementById("videoTitle").textContent = currentVideoTitle;
        document.getElementById("videoChannel").textContent = currentChannelName;
        videoInfo.style.display = "block";
      }
      startDigest(message.videoId, message.videoUrl || fallbackUrl, vid?.page || 1);
      sendResponse({ success: true });
      return false;
    }
    // Browser side panel: follow the active tab as usual.
    checkCurrentTab();
    sendResponse({ success: true });
  }
...
function handleFrontTabUrl(url) {
  if (!(url || "").match(/^https:\/\/(www\.)?bilibili\.com\/video\//)) {
    // In the browser side panel, close ourselves. In the in-page iframe the
    // host content script removes the overlay, so nothing else to do here.
    void saveCurrentTranscriptViewState();
    if (!YTD_IN_IFRAME) window.close();
    return;
  }

  const vid7 = extractVideoId(url);
  const newVideoId = vid7 ? vid7.bvid : null;
...
function setupEventListeners() {
  ...
  // Transcript actions
  document
    .getElementById("exportObsidianBtn")
    ?.addEventListener("click", exportTranscriptToObsidian);
  ...
  // 字幕轨道下拉：切换后重新拉取该轨道字幕
  document.getElementById("trackSelect")?.addEventListener("change", onTrackChange);
...
async function checkCurrentTab() {
  try {
    ...
    debugLog("[Bili Digest Panel] Found tab:", tab?.id, tab?.url);
    ...
    if (vid) {
      ...
      try {
        ...
        debugLog("[Bili Digest Panel] getVideoInfo result:", result);
        ...
        console.error("[Bili Digest Panel] getVideoInfo error:", e);
        currentVideoTitle = "";
        currentChannelName = "";
...
function extractVideoId(url) {
  try {
    ...
    // Bilibili video page: /video/BVxxxxxxxxxx
    const match = urlObj.pathname.match(/^\/video\/(BV[0-9A-Za-z]{10})/);
    if (match) {
      const page = Number(urlObj.searchParams.get("p") || "1");
...
async function startDigest(videoId, videoUrl) {
  ...
  if (cached) {
    ...
    currentChapters = Array.isArray(cached.chapters) ? cached.chapters : [];
    ...
    currentVideoDescription = cached.videoDescription || currentVideoDescription;
  ...
  const transcriptResult = await chrome.runtime.sendMessage({
    ...
    videoId: videoId,
    pageIndex: pageIndexFromUrl(currentVideoUrl),
    subtitleTrackLan: currentTrackLan || "",
  });
  ...
  if (!transcriptResult.success) {
    if (transcriptResult.error === "NO_SUPADATA_KEY") {
      showError(
        "字幕不可用",
        "Transcripts are available without an API key in Bili Digest.",
      );
      return;
    }
    showError(
  ...
  currentChapters = Array.isArray(transcriptResult.chapters) ? transcriptResult.chapters : [];
  currentTrackList = Array.isArray(transcriptResult.availableTracks) ? transcriptResult.availableTracks : [];
  if (transcriptResult.selectedTrackLan) currentTrackLan = transcriptResult.selectedTrackLan;
  ...
  // Bilibili meta returned by background (authoritative for the current page)
  if (transcriptResult.videoTitle) currentVideoTitle = transcriptResult.videoTitle;
  if (transcriptResult.channelName) currentChannelName = transcriptResult.channelName;
  if (transcriptResult.videoDescription) currentVideoDescription = transcriptResult.videoDescription;
  if (transcriptResult.videoDuration) currentVideoDuration = transcriptResult.videoDuration;

  // Wait until the authoritative title/UP are known before revealing the header,
  // so the video info shows reliably even when we started with empty metadata.
  if (currentVideoTitle || currentChannelName) {
  ...
  renderTranscript();
  applyLanguageControlsVisibility();
  showState("results");
  document.getElementById("tabsNav").style.display = "flex";
  restorePendingTranscriptViewState(videoId);
...
function renderLocalizedContent(text, surface, id) {
  ...
  const translation = translated
    ...
    : interfaceTranslationFailures.has(cacheKey)
      ? '<span class="translation-error">字幕翻译不可用。</span>'
      : '<span class="translation-pending">翻译中...</span>';
  ...
  try {
    for (
      ...
      try {
        ...
        console.error("[Bili Digest] Interface batch error:", error);
    ...
    console.error("[Bili Digest] Interface translation error:", error);
...
function renderAnalysisResults(analysis) {
  ...
  (analysis.chapters || []).forEach((chapter, index) => {
    ...
    li.addEventListener("click", () => {
      debugLog(
        "[Bili Digest Panel] Chapter clicked:",
        chapter.timestamp,
    ...
    div.innerHTML = `
      ...
      <div class="quote-meta">
        ...
        <div class="quote-actions">
          <button class="quote-save-note-btn" title="保存该引言为笔记">笔记</button>
          <button class="quote-copy-btn" title="复制该引言">复制</button>
        </div>
      </div>
    `;
    div.addEventListener("click", () => {
      debugLog(
        "[Bili Digest Panel] Quote clicked:",
        quote.timestamp,
...
async function saveQuoteAsNote(quote, btn) {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[Bili Digest] Save quote as note failed:", result.error);
      btn.textContent = "出错";
      setTimeout(() => {
        btn.textContent = originalText;
        btn.disabled = false;
    ...
    console.error("[Bili Digest] Save quote as note error:", error);
    btn.textContent = "出错";
    setTimeout(() => {
      btn.textContent = originalText;
      btn.disabled = false;
...
function populateTrackSelector() {
  ...
  if (!bar || !sel) return;
  const list = Array.isArray(currentTrackList) ? currentTrackList : [];
  if (!list.length) {
    bar.style.display = "none";
    return;
  }
  bar.style.display = "flex";
  const cur = currentTrackLan || "";
  sel.innerHTML = "";
  list.forEach((t) => {
    ...
    if (t.lan === cur) opt.selected = true;
    sel.appendChild(opt);
  });
}
...
function onTrackChange(event) {
  ...
  if (!value || value === currentTrackLan) return;
  currentTrackLan = value;
  void refetchTranscriptForTrack();
}

async function refetchTranscriptForTrack() {
  if (!currentVideoId) return;
  showState("loading");
  updateLoading("正在获取字幕", "");
  const result = await chrome.runtime.sendMessage({
  ...
  if (!result || !result.success) {
    showError("未找到字幕", (result && (result.message || result.error)) || "拉取失败");
    return;
  }
  currentTranscript = result.transcript;
  currentChapters = Array.isArray(result.chapters) ? result.chapters : [];
  ...
  currentTrackList = Array.isArray(result.availableTracks) ? result.availableTracks : [];
  populateTrackSelector();
  renderTranscript();
  applyLanguageControlsVisibility();
  showState("results");
...
function exportTranscript() {
  ...
  const videoUrl = `https://www.bilibili.com/video/${currentVideoId}`;
  ...
  exportText += `TRANSCRIPT:\n\n${transcriptContent}\n`;
  exportText += `\n${"—".repeat(60)}\n`;
  exportText += `Exported by Bili Digest\n`;
...
/**
 * Build a markdown document for the current transcript (reused by Obsidian export).
 */
function generateTranscriptMarkdown() {
  const transcriptContent = getDisplayedTranscriptText();
  const videoUrl = `https://www.bilibili.com/video/${currentVideoId}`;
  return `---
title: "${String(currentVideoTitle || "Bili Digest").replace(/"/g, '\"')}"
channel: "${String(currentChannelName || "").replace(/"/g, '\"')}"
url: "${videoUrl}"
videoId: "${String(currentVideoId || "").replace(/"/g, '\"')}"
created: "${new Date().toISOString()}"
---

# ${String(currentVideoTitle || "Bili Digest").replace(/"/g, '\"')}

${currentVideoDescription ? `> ${String(currentVideoDescription).split("\n").join("\n> ")}` : ""}
...
/**
 * Export the full transcript + notes to Obsidian via the Local REST API.
 */
async function exportTranscriptToObsidian() {
  const btn = document.getElementById("exportObsidianBtn");
  if (btn) btn.disabled = true;
  try {
    const transcriptText = getDisplayedTranscriptText();
    const notesThisVideo = (currentNotes || []).filter((n) => !n.videoId || n.videoId === currentVideoId);
    const resp = await chrome.runtime.sendMessage({
      action: "exportTranscriptToObsidian",
      videoId: currentVideoId,
      videoTitle: currentVideoTitle,
      channelName: currentChannelName,
      videoDescription: currentVideoDescription,
      language: currentTranscriptLanguage,
      timestamped: (currentTranscript || []).map((e) => ({
        start: e.start,
        text: e.text,
      })),
      chapters: currentChapters || [],
      notes: notesThisVideo,
      analysis: currentAnalysis || null,
      transcriptText,
      page: pageIndexFromUrl(currentVideoUrl),
      duration: currentVideoDuration || 0,
      filename: `${currentVideoTitle || currentVideoId || "digest"} - ${currentVideoId || ""}`,
    });
    exportObsidianFeedback(resp, btn);
  } catch (err) {
    exportObsidianFeedback({ success: false, error: err.message }, btn);
  } finally {
 ...
 */
async function exportNoteToObsidian(note) {
  const resp = await chrome.runtime.sendMessage({
    action: "exportNoteToObsidian",
    videoId: note.videoId,
    videoTitle: note.videoTitle,
    channelName: note.channelName,
    note,
  });
  exportObsidianFeedback(resp, null);
  return resp;
}

/**
 * Show success / error feedback for an Obsidian export. When the config is
 * missing we point the user to Settings.
 */
function exportObsidianFeedback(resp, btn) {
  const shown = btn || null;
  if (resp && resp.success) {
    if (shown) {
      const old = shown.textContent;
      shown.textContent = "已导出到 Obsidian";
      setTimeout(() => { shown.textContent = old; }, 2000);
    }
  } else {
    const msg = resp?.code === "NO_OBSIDIAN_CONFIG"
      ? "未配置 Obsidian。打开设置填写地址/密钥。"
      : (resp?.error || "Obsidian 导出失败");
    console.warn("[Bili Digest] Obsidian export error:", msg);
    if (shown) {
      const old = shown.textContent;
      shown.textContent = msg;
      setTimeout(() => { shown.textContent = old; }, 3000);
    }
  }
}
...
function showState(state) {
  ...
  if (state === "results") {
    ...
    document.getElementById("transcriptModeControl").style.display = "none";
  }
...
function showConfigError(configStatus) {
  ...
  showState("error");
  document.getElementById("errorTitle").textContent = "AI 未配置";
  document.getElementById("errorMessage").textContent =
    "字幕已可用（B站原生字幕，无需密钥）。如需 AI 概览 / 双语翻译 / 解释，请在设置中填写 DeepSeek API 密钥。";
...
async function triggerAnalysis() {
  ...
  if (chapterList)
    chapterList.innerHTML =
      '<li class="chapter-item" style="color: var(--text-muted); border: none;">加载章节中...</li>';
  if (quotesList)
    quotesList.innerHTML =
      '<div class="quote-item" style="color: var(--text-muted); border-left-color: var(--border);">加载引言中...</div>';
  ...
  try {
    ...
    console.error("[Bili Digest Panel] Analysis error:", error);
    if (chapterList)
...
async function seekTo(seconds) {
  debugLog("[Bili Digest Panel] seekTo called with:", seconds);
  if (seconds === undefined || seconds === null) {
    debugLog("[Bili Digest Panel] seekTo aborted - no seconds value");
    return;
  }

  // P0-3: 统一走后端 relayToContent（由其按 sender.tab 命中当前 B站 页面），
  // 不再依赖可能过期的 youtubeTabId 直连，失败时输出可观测日志。
  try {
    const result = await chrome.runtime.sendMessage({
      action: "relayToContent",
      payload: {
    ...
    if (!result || result.success !== true) {
      console.warn("[Bili Digest Panel] seekTo 转发未成功:", result);
    }
  } catch (error) {
    console.error("[Bili Digest Panel] seekTo error:", error);
  }
}
  ...
  tooltip
    ...
    .addEventListener("click", async (event) => {
      ...
      try {
        ...
        console.error("[Bili Digest] Save selected note error:", error);
...
async function evictOldCacheEntries(maxEntries) {
  try {
    ...
    if (toRemove.length > 0) {
      ...
      debugLog(`[Bili Digest] Evicted ${toRemove.length} old cache entries`);
    }
  } catch (error) {
...
async function loadNotes(videoId) {
  try {
    ...
    console.error("[Bili Digest Panel] Load notes error:", error);
  }
}
...
function renderNotes(notes, filteredVideoId) {
  ...
  if (!notes || notes.length === 0) {
    ...
    notesIntro.textContent = filteredVideoId ? "暂无笔记" : "还没有保存笔记";
    return;
  }
...
async function deleteNote(noteId) {
  try {
    ...
    console.error("[Bili Digest Panel] Delete note error:", error);
  }
}
...
async function playbackTrackingTick() {
  try {
    ...
    // P0-2: 定位不到视频时跳过本轮，避免 currentTime 回落 0 把字幕钉回第一句。
    if (result.response.found === false) return;
...
async function loadTranscriptViewState(videoId) {
  ...
  try {
    ...
    console.error("[Bili Digest] Reading position load error:", error);
    return null;
  }
}
...
async function saveTranscriptViewState(videoId, scrollTop) {
  ...
  try {
    ...
    console.error("[Bili Digest] Reading position save error:", error);
  }
}
...
/**
 * Non-Chinese source subtitles can offer translation; Chinese sources get the
 * original text only (no token-wasting translation options).
 */
function supportsTranslation() {
  return currentTranscriptLanguage !== "zh";
}

/**
 * Show/hide the Original/中文/双语 language control based on the detected
 * source language. Chinese sources keep the control hidden.
 */
function applyLanguageControlsVisibility() {
  const control = document.getElementById("transcriptModeControl");
  if (!control) return;
  if (supportsTranslation()) {
    control.style.display = "inline-flex";
    if (currentTranscriptMode !== "original") {
      // Non-Chinese: allow whatever the user previously chose.
      setTranscriptModeButtons(currentTranscriptMode);
    }
  } else {
    control.style.display = "none";
    // Chinese source: force original, never show translation modes.
    if (currentTranscriptMode !== "original") {
      currentTranscriptMode = "original";
      setTranscriptModeButtons("original");
    }
...
function alignTranslatedSegmentBatch(sourceSegments, responseSegments) {
  ...
  return sourceSegments.map((segment) => ({
    ...
    error: translatedById.has(segment.id) ? "" : "字幕翻译不可用。",
  }));
}
