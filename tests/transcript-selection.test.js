...
test("the selection toolbar preserves selection and contains pointer events", () => {
  ...
  assert.match(
    source,
    /tooltip\.addEventListener\("click", \(event\) => \{\s+event\.stopPropagation\(\);/,
  );
  assert.match(
    source,
...
test("the transcript view restores without resuming automatic scrolling", () => {
  assert.match(
    source,
    /void saveCurrentTranscriptViewState\(\);\s+if \(!YTD_IN_IFRAME\) window\.close\(\);/,
  );
  assert.match(
    source,
...
test("the panel never borrows a background Bilibili tab", () => {
  assert.match(
    source,
  ...
  assert.match(
    source,
    /!tab\.url\.match\(.*bilibili.*\)\) \{/,
  );
  assert.match(source, /handleFrontTabUrl\(tab\.url\);\s*return;/);
  assert.doesNotMatch(
    source,
    /chrome\.tabs\.query\(\{ url: "https:\/\/www\.bilibili\.com\/\*" \}\)/,
  );
});
