...
function loadBackgroundHelpers({
  ...
  const sandbox = {
    ...
    YTD_SETTINGS: {
      ...
      canonicalBilibiliUrl: (videoId, seconds = 0) =>
        `https://www.bilibili.com/video/${videoId}${seconds ? `?t=${seconds}` : ""}`,
    },
  };
  sandbox.globalThis = sandbox;
...
test("selected transcript notes keep exact text and row timestamp", async () => {
  ...
  assert.equal(
    ...
    "https://www.bilibili.com/video/BV1xx411c7mD?t=92",
  );
});
...
test("structured translation batches align by stable ID and expose missing fallback", () => {
  ...
  assert.match(aligned[0].error, /字幕翻译不可用/u);
